# EXAMPLE 5: 10 LATENT VARIABLES, ALL TRIPLETS, 'LOOP' STRUCTURE, Y & Z residual correlations
...
for (i in seq(1, max(edges$node_to), 3)) {
  ...
  df_temp <- subset(edges, (node_from >= i & node_from <= i+2 & node_to >= i & node_to <= i+2) #edges within triplets
                    | node_from == i+2 & node_to == i+4) #to include edges of A.z to B.y, B.z to C.y, etc. 
  ...
  if (i == max(edges$node_to) - 2){
    ...
    #add the row connecting the first triplet with the last triplet (A.y to J.z)
    edges_programmed <- rbind(edges_programmed, subset(edges, (node_from == min(edges$node_from) + 1 & node_to == i+2)))
  }
}
