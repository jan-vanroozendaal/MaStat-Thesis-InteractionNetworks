...
for (i in seq(1, max(edges$node_to), 9)) {
  ...
  df_temp <- subset(edges, (node_from >= i & node_from <= i+2 & node_to >= i & node_to <= i+2) #edges within triplet A/D/G
                    ...
                    | (node_from == i+2 & node_to == i+5) #to include edges of A.z to B.z, D.z to E.z, G.z to H.z
                    | (node_from == i+5 & node_to == i+8) #to indluce edges of B.z to C.z, E.z to F.z, H.z to I.z
                    | (node_from == i+2 & node_to == i+8)) #to include edges of A.z to C.z, D.z to F.z, G.z to I.z
