...
for (i in 1:nrow(df_edgelist)) {
  ...
  for (j in seq(1, max(df_edgelist$node_to), 3)) {
    ...
    if (
      ...
      | (df_edgelist$node_from[i] == j+2 & df_edgelist$node_to[i] == j+5) #edges of A.z to B.z, B.z to C.z, etc.(first loop) 
      | (df_edgelist$node_from[i] == j+5 & df_edgelist$node_to[i] == j+8) #edges of A.z to C.z, B.z to D.z, etc (second loop)
      | (df_edgelist$node_from[i] == 3 & df_edgelist$node_to[i] == 27)) #edge A.z to I.z, cannot be obtained from step size
      
    {
      ...
      if  ((df_edgelist$node_from[i] == min(df_edgelist$node_from) + 2 & df_edgelist$node_to[i] == j + 2)
           | (df_edgelist$node_from[i] == min(df_edgelist$node_from) + 5 & df_edgelist$node_to[i] == j + 2))
      {
        df_edgelist$programmed[i] <- 1
      }
    }
