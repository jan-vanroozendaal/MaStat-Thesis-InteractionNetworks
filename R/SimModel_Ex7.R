# EXAMPLE 7: 10 LATENT VARIABLES, ALL TRIPLETS, 'DOUBLE LOOP' STRUCTURE
...
for (i in seq(1, max(edges$node_to), 3)) {
  ...
  df_temp <- subset(edges, (node_from >= i & node_from <= i+2 & node_to >= i & node_to <= i+2) #edges within triplets
                    | (node_from == i+2 & node_to == i+5) #to include edges of A.z to B.z, B.z to C.z, etc. (first loop)
                    | (node_from == i+2 & node_to == i+8) #to include edges of A.z to C.z, B.z to D.z, etc. (second loop)
                    | (node_from == 3 & node_to == 27)) #to include edge A.z to I.z, as it cannot be obtained from step size
  #append these rows to the dataframe 'edges_programmed'
  edges_programmed <- rbind(edges_programmed, df_temp)
  
  if (i == max(edges$node_to) - 2){
    #if reached at the last iteration of the loop
    #add the row connecting the first triplet with the last triplet (A.z to J.z) (first loop)
    #add the row connecting the second triplet with the last triplet (B.z to J.z) (second loop)
    edges_programmed <- rbind(edges_programmed, subset(edges, ((node_from == min(edges$node_from) + 2 & node_to == i+2)
                                                               |(node_from == min(edges$node_from) + 5 & node_to == i+2))))
  }
}
