...
for (i in 1:nrow(df_edgelist)) {
  ...
  for (j in seq(1, max(df_edgelist$node_to), 3)) {
    ...
    if (
      ...
      | df_edgelist$node_from[i] == j+2 & df_edgelist$node_to[i] == j+4) #to include edges of A.z to B.y, B.z to C.y, etc.  
      
    {
      ...
      if  (df_edgelist$node_from[i] == min(df_edgelist$node_from) + 1 & df_edgelist$node_to[i] == j + 2)
      {
        df_edgelist$programmed[i] <- 1
      }
    }
