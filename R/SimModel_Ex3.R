...
df <- data.frame(ii_score=as.numeric(9),
...
edges_programmed <- data.frame(node_from=as.integer(),
                               node_to=as.integer(),
                               weight=as.numeric())

#step size of 9, to represent each set of 3 triplets 
for (i in seq(1, max(edges$node_to), 9)) {
  #find the edges that occur within a triplet and as programmed in the model
  df_temp <- subset(edges, (node_from >= i & node_from <= i+2 & node_to >= i & node_to <= i+2) #edges within triplet A/D/G
                    | (node_from >= i+3 & node_from <= i+5 & node_to >= i+3 & node_to <= i+5) #edges within triplet B/E/H
                    | (node_from >= i+6 & node_from <= i+8 & node_to >= i+6 & node_to <= i+8) #edges within triplet C/F/I
                    | (node_from == i+2 & node_to == i+4) #to include edges of A.z to B.y, D.z to E.y, G.z to H.y
                    | (node_from == i+4 & node_to == i+2) #flipped
                    | (node_from == i+2 & node_to == i+7) #to include edges of A.z to C.y, D.z to F.y, G.z to I.y
                    | (node_from == i+7 & node_to == i+2) #flipped
                    | (node_from == i+5 & node_to == i+1) #to include edges of B.z to A.y, E.z to D.y, H.z to G.y
                    | (node_from == i+1 & node_to == i+5) #flipped
                    | (node_from == i+5 & node_to == i+7) #to indluce edges of B.z to C.y, E.z to F.y, H.z to I.y
                    | (node_from == i+7 & node_to == i+5) #flipped
                    | (node_from == i+8 & node_to == i+1) #to include edges of C.z to A.y, F.z to D.y, I.z to G.y
                    | (node_from == i+1 & node_to == i+8) #flipped
                    | (node_from == i+8 & node_to == i+4) #to include edges of C.z to B.y, G.z to E.y, I.z to H.y
                    | (node_from == i+4 & node_to == i+8)) #flipped
  
                    
  #append these rows to the dataframe 'edges_programmed'
  edges_programmed <- rbind(edges_programmed, df_temp)
}

#dplyr package used for the function 'anti_join'
#returns subset of complete edgelist dataframe minus the ones found within triplets and intentionally programmed in model
#these are the spurious correlations with threshold level found in the qgraph() function
edges_spurious <- anti_join(edges, edges_programmed)
