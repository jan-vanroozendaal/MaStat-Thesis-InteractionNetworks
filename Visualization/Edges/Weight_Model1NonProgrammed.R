...
#subset on Model
df_weight_model <- merge(x=df_nonprogrammed_total[df_nonprogrammed_total$model == "Model 1"
...
for (i in 1:length(thresholds)) {
  ...
  centers <- as.data.frame(kmeans(df_subset[,c(3,8)], centers = 5)$centers)
  
  # order the centers
  centers <- centers[order(centers$occur, centers$avg_weight),]
  
  # call kmeans again but this time passing the centers calculated in the previous step
  clusteridx <- kmeans(x = df_subset[,c(3,8)], centers = centers)$cluster
...
#fix cluster order in 0.035 threshold
df_plot$cluster[df_plot$threshold == "0.035" & df_plot$cluster == "1"] <- "2a"
df_plot$cluster[df_plot$threshold == "0.035" & df_plot$cluster == "2"] <- "1"
df_plot$cluster[df_plot$threshold == "0.035" & df_plot$cluster == "2a"] <- "2"

#fix cluster oder in 0.05 threshold
df_plot$cluster[df_plot$threshold == "0.05" & df_plot$cluster == "3"] <- "4a"
df_plot$cluster[df_plot$threshold == "0.05" & df_plot$cluster == "4"] <- "3"
df_plot$cluster[df_plot$threshold == "0.05" & df_plot$cluster == "4a"] <- "4"
...
for (i in 1:length(thresholds)) {
  #create file name
  plot_name <- paste(paste(i,"_Model1_NonProgrammedEdgeCat_", sep=""), paste(thresholds[i], ".jpg", sep=""), sep="")
