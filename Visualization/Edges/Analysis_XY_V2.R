#subset by Model 2 and 3, spurious only
...
#subset from df_nonprogrammed_total by Model 5
df_subtotal <- df_nonprogrammed_total[df_nonprogrammed_total$model == "Model 5"
...
df_subtotal <- df_subtotal[df_subtotal$path_type == "X to Y"
                           & df_subtotal$distance == 1,]

df_subtotal <- df_subtotal[(df_subtotal$node_from == "A.y" & df_subtotal$node_to == "J.x")
                           |substr(df_subtotal$node_to, 3, 3) == "y" ,]

df_subtotal <- df_subtotal[df_subtotal$path_name != "A.x to J.y",]


#append rows to df_total
df_total <- rbind(df_total, df_subtotal[,c(1,2,3,4,5,6,7)])
...
#filter by path type X-Y
df_total <- df_total[df_total$path_type == "X to Y",]
...
for (i in 1:length(models)) {
  ...
  #create file name
  plot_name <- paste(models[i], "XY_Condensed.jpg", sep = "")
  ...
  file_name_part2 <- paste(file_name_part1, "X-Y_V2", sep="")
  file_name_part3 <- paste(file_name_part2, ".csv", sep="")
    
    
  write.csv(df_result, paste("C:\\Users\\janvr\\Desktop\\Visualization\\Edges\\X-Y\\", file_name_part3, sep=""), row.names = FALSE)
}