#subset by Models 2 & 6 as the edges all happen within cluster
...
#subset from df_nonprogrammed_total by Model 4 and 7
...
#filter by path type Y-Z
df_total <- df_total[df_total$path_type == "Y to Z",]
...
for (i in 1:length(models)) {
  ...
  #create file name
  plot_name <- paste(models[i], "YZ_Condensed.jpg", sep = "")
  ...
  file_name_part2 <- paste(file_name_part1, "Y-Z_V2", sep="")
  file_name_part3 <- paste(file_name_part2, ".csv", sep="")
    
    
  write.csv(df_result, paste("C:\\Users\\janvr\\Desktop\\Visualization\\Edges\\Y-Z\\", file_name_part3, sep=""), row.names = FALSE)
}