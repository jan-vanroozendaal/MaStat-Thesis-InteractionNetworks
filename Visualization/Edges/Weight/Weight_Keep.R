...
#subset on Model
df_weight_model <- merge(x=df_nonprogrammed_total[df_nonprogrammed_total$model == "Model 2"
                                                  & df_nonprogrammed_total$occur != 0,],
                         y=df_weight_total, all.x=TRUE)
row.names(df_weight_model) <- NULL

df_weight_model <- df_weight_model[abs(df_weight_model$avg_weight) > 0.005,]
...
library(ggplot2)
bp2 <- ggplot(df_plot, aes(x=occur, y=avg_weight, colour=cluster)) + 
  geom_point()

bp2 + facet_grid(threshold ~ case, scales = "free_y")