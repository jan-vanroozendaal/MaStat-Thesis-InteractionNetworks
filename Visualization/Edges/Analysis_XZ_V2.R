#subset by Models 2, 3, 6 as they all happen within cluster
df_total <- df_spurcluster_total[(df_spurcluster_total$model =="Model 2"
                                  | df_spurcluster_total$model =="Model 3"
                                  | df_spurcluster_total$model == "Model 6")
                                 & df_spurcluster_total$occur != 0,]


#subset from df_nonprogrammed_total by Model 4, 5 and 7
df_subtotal <- df_nonprogrammed_total[(df_nonprogrammed_total$model == "Model 4"
                                       | df_nonprogrammed_total$model == "Model 5"
                                       | df_nonprogrammed_total$model == "Model 7")
                                      & df_nonprogrammed_total$occur != 0,]


#use distance metric for df_subtotal
alphabet <- "ABCDEFGHIJ"

for (i in 1:nrow(df_subtotal)) {
  start <- substr(df_subtotal$path_name[i], 1, 1)
  end <- substr(df_subtotal$path_name[i], 8, 8)
  
  start_idx <- gregexpr(pattern=start, alphabet)[[1]][1]
  end_idx <- gregexpr(pattern=end, alphabet)[[1]][1]
  
  if (end_idx - start_idx == nchar(alphabet) - 1) {
    df_subtotal$distance[i] <- 1
  }
  else {
    df_subtotal$distance[i] <- end_idx - start_idx
  }
}

#find path type in df_subtotal
for (i in 1:nrow(df_subtotal)) {
  if (substr(df_subtotal$path_name[i],3,3) == "x" & substr(df_subtotal$path_name[i],10,10) == "y"
      | substr(df_subtotal$path_name[i],3,3) == "y" & substr(df_subtotal$path_name[i],10,10) == "x") {
    df_subtotal$path_type[i] <- "X to Y"
  }
  else if (substr(df_subtotal$path_name[i],3,3) == "x" & substr(df_subtotal$path_name[i],10,10) == "z"
           | substr(df_subtotal$path_name[i],3,3) == "z" & substr(df_subtotal$path_name[i],10,10) == "x") {
    df_subtotal$path_type[i] <- "X to Z"
  }
  else if (substr(df_subtotal$path_name[i],3,3) == "y" & substr(df_subtotal$path_name[i],10,10) == "z"
           | substr(df_subtotal$path_name[i],3,3) == "z" & substr(df_subtotal$path_name[i],10,10) == "y") {
    df_subtotal$path_type[i] <- "Y to Z"
  }
  else if (substr(df_subtotal$path_name[i],3,3) == "x" & substr(df_subtotal$path_name[i],10,10) == "x") {
    df_subtotal$path_type[i] <- "X to X"
  }
  else if (substr(df_subtotal$path_name[i],3,3) == "y" & substr(df_subtotal$path_name[i],10,10) == "y") {
    df_subtotal$path_type[i] <- "Y to Y"
  }
  else if (substr(df_subtotal$path_name[i],3,3) == "z" & substr(df_subtotal$path_name[i],10,10) == "z") {
    df_subtotal$path_type[i] <- "Z to Z"
  }
}

#filtering steps to get relevant edges

# (A) Model 4
df_sub_model4 <- df_subtotal[df_subtotal$model == "Model 4"
                             & df_subtotal$path_type == "X to Z"
                             & df_subtotal$distance == 1
                             ,]


# (B) Model 5
df_sub_model5 <- df_subtotal[df_subtotal$model == "Model 5"
                             & df_subtotal$path_type == "X to Z"
                             & df_subtotal$distance == 1
                             ,]

df_sub_model5 <- df_sub_model5[(df_sub_model5$node_from == "A.x" & df_sub_model5$node_to == "J.z")
                               |substr(df_sub_model5$node_to, 3, 3) == "x" ,]
df_sub_model5 <- df_sub_model5[df_sub_model5$path_name != "A.z to J.x",]

# (C) Model 7
df_sub_model7 <- df_subtotal[df_subtotal$model == "Model 7"
                             & df_subtotal$path_type == "X to Z"
                             ,]


for (i in 1:nrow(df_sub_model7)) {
  start <- substr(df_sub_model7$path_name[i], 1, 1)
  end <- substr(df_sub_model7$path_name[i], 8, 8)
  
  start_idx <- gregexpr(pattern=start, alphabet)[[1]][1]
  end_idx <- gregexpr(pattern=end, alphabet)[[1]][1]
  
  if (end_idx - start_idx == nchar(alphabet) - 1) {
    df_sub_model7$distance[i] <- 1
  }
  else if (end_idx - start_idx == nchar(alphabet) - 2) {
    df_sub_model7$distance[i] <- 2
  }
  else {
    df_sub_model7$distance[i] <- end_idx - start_idx
  }
}

df_sub_model7 <- df_sub_model7[df_sub_model7$distance < 3 ,]


#append rows to df_total
df_total <- rbind(df_total, df_sub_model4[,c(1,2,3,4,5,6,7)])
df_total <- rbind(df_total, df_sub_model5[,c(1,2,3,4,5,6,7)])
df_total <- rbind(df_total, df_sub_model7[,c(1,2,3,4,5,6,7)])

#again, find path types
...
#filter by path type X-Z
df_total <- df_total[df_total$path_type == "X to Z",]
...
for (i in 1:length(models)) {
  ...
  #create file name
  plot_name <- paste(models[i], "XZ_Condensed.jpg", sep = "")
  ...
  file_name_part2 <- paste(file_name_part1, "X-Z_V2", sep="")
  file_name_part3 <- paste(file_name_part2, ".csv", sep="")
    
    
  write.csv(df_result, paste("C:\\Users\\janvr\\Desktop\\Visualization\\Edges\\X-Z\\", file_name_part3, sep=""), row.names = FALSE)
}