...
median_col <- aggregate(df_total$`%_programmed`, by = list(df_total$model, df_total$case, df_total$threshold), median)
Q1_col <- aggregate(df_total$`%_programmed`, by = list(df_total$model, df_total$case, df_total$threshold), quantile, probs = 0.25)
Q3_col <- aggregate(df_total$`%_programmed`, by = list(df_total$model, df_total$case, df_total$threshold), quantile, probs = 0.75)
...
for (i in 1:length(models)) {
  ...
  file_name_part2 <- paste(file_name_part1, "ProgrammedKPI", sep="")
