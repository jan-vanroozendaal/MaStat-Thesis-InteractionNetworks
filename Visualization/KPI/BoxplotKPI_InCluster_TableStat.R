...
#subset by Model 2, 3, and 6
df_total <- df_total[df_total$model == "Model 2" | df_total$model == "Model 3" | df_total$model == "Model 6",]

median_col <- aggregate(df_total$`%_incluster`, by = list(df_total$model, df_total$case, df_total$threshold), median)
Q1_col <- aggregate(df_total$`%_incluster`, by = list(df_total$model, df_total$case, df_total$threshold), quantile, probs = 0.25)
Q3_col <- aggregate(df_total$`%_incluster`, by = list(df_total$model, df_total$case, df_total$threshold), quantile, probs = 0.75)
...
for (i in 1:length(models)) {
  ...
  file_name_part2 <- paste(file_name_part1, "ClusterKPI", sep="")
  file_name_part3 <- paste(file_name_part2, ".csv", sep="")
  
  write.csv(df_result, paste("C:\\Users\\janvr\\Desktop\\KPITables\\", file_name_part3, sep=""), row.names = FALSE)
}