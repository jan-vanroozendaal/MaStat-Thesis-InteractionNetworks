...
library(ggplot2)
bp <- ggplot(df_total, aes(x=threshold, y=df_total$size, group=threshold)) + 
  geom_boxplot(aes(fill=threshold))

# Split in horizontal direction
bp + scale_y_continuous(breaks = seq(0, 250, 100)) + 
  labs(x = "Threshold Level", y = "Size", fill="Threshold Level") +
