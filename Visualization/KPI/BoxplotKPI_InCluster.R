...
library(ggplot2)
bp <- ggplot(df_total[df_total$model == "Model 2"
                      | df_total$model == "Model 3"
                      | df_total$model == "Model 6",], aes(x=threshold, y=`%_incluster`, group=threshold)) + 
  geom_boxplot(aes(fill=threshold))

# Split in horizontal direction
bp + scale_y_continuous(breaks = seq(0, 1, 0.5), limits = c(0,1)) + 
  labs(x = "Threshold Level", y = "% in cluster (scaled 0-1)", fill="Threshold Level") +
